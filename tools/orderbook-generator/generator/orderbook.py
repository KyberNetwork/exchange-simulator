...
TIMESTAMPS = range(TIMESTAMP_START, TIMESTAMP_STOP, TIMESTAMP_STEP)
    ...
    for exchange in EXCHANGES:
        for token in TOKENS:
            for timestamp in TIMESTAMPS:
                books[_prepare_book_name(exchange, token, BASE_TOKEN, timestamp)] = prepare_single_order_book(
                    min_quantity=MIN_QUANTITY, max_quantity=MAX_QUANTITY, min_rate=MIN_RATE, max_rate=MAX_RATE,
                    middle_rate=MIDDLE_RATE, rate_gap=RATE_GAP, number_of_asks=NUMBER_OF_ASKS,
...
def prepare_single_order_book(min_quantity, max_quantity, min_rate, max_rate, middle_rate, rate_gap, number_of_asks,
                              number_of_bids):
