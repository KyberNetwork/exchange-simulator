...
def prepare_book_name(exchange, token, base_token, timestamp):
    return f"{exchange}_{token.lower()}_{base_token.lower()}_{timestamp}"


def prepare_random_order_book_from_params(params):
    return _prepare_single_randomized_order_book(min_quantity=params.min_quantity,
...
def _prepare_single_randomized_order_book(min_quantity, max_quantity, min_rate, max_rate, middle_rate, rate_gap,
...
class OrderBookGenerator:
    ...
    def __str__(self):
        return f"{self.__class__.__name__}(start={self.params.timestamp_start}, stop={self.params.timestamp_stop})"
    ...
    async def prepare_books(self):
        ...
        for exchange in self.params.exchanges:
            for token in self.params.tokens:
                for timestamp in range(self.params.timestamp_start,
                                       self.params.timestamp_stop,
                                       self.params.timestamp_step):
                    books[prepare_book_name(exchange, token, self.params.base_token, timestamp)] = \
...
class BlockRandomOrderBookGenerator(OrderBookGenerator):
    ...
    async def prepare_books(self):
        ...
        for exchange in self.params.exchanges:
            for token in self.params.tokens:
                timestamp_range = range(self.params.timestamp_start,
                                        self.params.timestamp_stop,
                                        self.params.timestamp_step)
                ...
                for timestamp in timestamp_range:
                    ...
                    books[prepare_book_name(exchange, token, self.params.base_token, timestamp)] = order_book
