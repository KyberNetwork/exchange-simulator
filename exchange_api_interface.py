...
class TradeOutput:
    ...
    def __init__(self, error, error_msg, order_id, received, remains, balance):
        """
        ...
        self.original_qty = received
        self.remaining_qty = remains
...
class LiquiApiInterface(ExchangeApiInterface):
    ...
    def check_answers(exchange_results, required_results):
        ...
        if not(all(p in answers for p in required_results)):
            return False, "missing required results"
    ...
    def parse_answers(exchange_results, required_results):
        """Converts the exchange results to liqui results
         ...
         exchange"""
        answers = vars(exchange_results)
        logger.debug("Exchange result: {}".format(answers))
        replies = {"return": {}}
        for key, value in answers.items():
    ...
    @staticmethod
    def translate_args(post_args, required_parameters):
        cleaned_post_args = {}
        if len(post_args) > 10:
        ...
        for key, value in post_args.items():
            ...
            if key == "timestamp":
                cleaned_post_args["timestamp"] = int(value)
